...
const ChartView = ({ width, height, minX, maxX, minY, maxY, radius, items, onClickChart }) => {
    ...
    const drawShape = (context, radius) => {
        ...
        // First quadrant: Right-angled triangle
        let nextPos = coordinateToPosition({ x: 0, y: 0 });
        context.moveTo(nextPos.x, nextPos.y);

        nextPos = coordinateToPosition({ x: 0, y: radius / 2 });
        ...
        // Second quadrant: Circle section
        context.beginPath();
        nextPos = coordinateToPosition({ x: 0, y: 0 });
        context.arc(nextPos.x, nextPos.y, getScaleFactorX() * radius, Math.PI, -Math.PI / 2 );
        context.lineTo(nextPos.x, nextPos.y);
        context.fill();
        context.stroke();
        ...
        nextPos = coordinateToPosition({ x: -radius / 2, y: 0 });
        context.lineTo(nextPos.x, nextPos.y);

        nextPos = coordinateToPosition({ x: -radius / 2, y: -radius });
        context.lineTo(nextPos.x, nextPos.y);
